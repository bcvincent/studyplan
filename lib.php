...
/**
 * Is a given scale used by the instance of studyplan?
 *
 * This function returns if a scale is being used by one studyplan
 * if it has support for grading and scales. Commented code should be
 * modified if necessary. See forum, glossary or journal modules
 * as reference.
 *
 * @param int $studyplanid ID of an instance of this module
 * @return bool true if the scale is used by the given studyplan instance
 */
function studyplan_scale_used($studyplanid, $scaleid) {
    global $DB;

    /** @example */
    if ($scaleid and $DB->record_exists('studyplan', array('id' => $studyplanid, 'grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
...
/**
 * Checks if scale is being used by any instance of studyplan.
 *
 * This is used to find out if scale used anywhere.
 *
 * @param $scaleid int
 * @return boolean true if the scale is used by any studyplan instance
 */
function studyplan_scale_used_anywhere($scaleid) {
    global $DB;

    /** @example */
    if ($scaleid and $DB->record_exists('studyplan', array('grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
...
/**
 * Creates or updates grade item for the give studyplan instance
 *
 * Needed by grade_update_mod_grades() in lib/gradelib.php
 *
 * @param stdClass $studyplan instance object with extra cmidnumber and modname property
 * @param mixed optional array/object of grade(s); 'reset' means reset grades in gradebook
 * @return void
 */
function studyplan_grade_item_update(stdClass $studyplan, $grades=null) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');

    /** @example */
    $item = array();
    $item['itemname'] = clean_param($studyplan->name, PARAM_NOTAGS);
    $item['gradetype'] = GRADE_TYPE_VALUE;
    $item['grademax']  = $studyplan->grade;
    $item['grademin']  = 0;

    grade_update('mod/studyplan', $studyplan->course, 'mod', 'studyplan', $studyplan->id, 0, null, $item);
}

/**
 * Update studyplan grades in the gradebook
 *
 * Needed by grade_update_mod_grades() in lib/gradelib.php
 *
 * @param stdClass $studyplan instance object with extra cmidnumber and modname property
 ...
 * @return void
 */
function studyplan_update_grades(stdClass $studyplan, $userid = 0) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');
    ...
    grade_update('mod/studyplan', $studyplan->course, 'mod', 'studyplan', $studyplan->id, 0, $grades);
}
 ...
 */
function studyplan_get_file_areas($course, $cm, $context) {
    return array();
}

/**
 * File browsing support for studyplan file areas
 *
 * @package mod_studyplan
 * @category files
 *
 ...
 */
function studyplan_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    return null;
}

/**
 * Serves the files from the studyplan file areas
 *
 * @package mod_studyplan
 * @category files
 *
 ...
 * @param stdClass $context the studyplan's context
 * @param string $filearea the name of the file area
 * @param array $args extra arguments (itemid, path)
 * @param bool $forcedownload whether or not force download
 * @param array $options additional options affecting the file serving
 */
function studyplan_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB, $CFG;
